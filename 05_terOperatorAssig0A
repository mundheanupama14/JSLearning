...
function greaterNumber(value1, value2) { // STEP-1.1 Function Name with STEP-1.2 Two Arguments
    ...
    // console.log(result);
    console.log(`The Gretest Number Is: ${result}`);
...
function isEvenOrOddNum(myNumber) { // STEP-2.1 Function Name with STEP-2.2 One Argument
    ...
    var result = (myNumber % 2 == 0) ? true : false; // Even - Divide by 2 and reminder is 0 
    return result; // S1TEP-2.2 Return Value True or False
...
function wordLength(word) { // STEP-3.1 Function Name with STEP-3.2 One Argument
    ...
    var result = wordLength % 2 == 0 ? "Even" : "Odd"; // Even or Odd length
    return result; // STEP-3.2 Return Value Even or Odd
}
